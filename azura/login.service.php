<?php
	...
	class LoginService{
		...
		public function login(){
			$query = 'select id, name from users where email = :email and password = md5(:password)';
		...
		public function register(){
			...
			$query = 'insert into users(name,email,password,confirm_password)values(:name,:email,md5(:password),md5(:confirm_password))';
			$stmt = $this->connection->prepare($query);
		...
		public function recover(){
			$query = 'select name, email, password, confirm_password from users';
			$stmt = $this->connection->prepare($query);
			$stmt->execute();
